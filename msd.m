...
figure
grid on
fanimator(@(t) plot(x_pos(t),0,'s','MarkerFaceColor','k','MarkerSize',20),'AnimationRange',[0 10]);
hold on;
fanimator(@(t) plot([-1.5 x_pos(t)],[0 0],'k-'),'AnimationRange',[0 10]);
fanimator(@(t) text(-0.3,0.5,"Timer: "+num2str(t,2)+" s"),'AnimationRange',[0 10]);
fanimator(@(t) text(-0.3,0.3,"Position: "+num2str(x_pos(t),2)+" m"),'AnimationRange',[0 10]);
playAnimation
